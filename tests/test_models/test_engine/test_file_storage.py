...
class TestBmFsInstances(unittest.TestCase):
    ...
    def test_all(self):
        """checks if all() func returns newly created instance"""
    ...
    def test_reload(self):
        """checks proper usage of reload func"""
        os.remove(F)
        self.bm_obj.save()
        bm_id = self.bm_obj.id
        actual = 0
    ...
    def test_save_reload_class(self):
        """checks proper usage of class attr in file storage"""
        os.remove(F)
        self.bm_obj.save()
        bm_id = self.bm_obj.id
        actual = 0
